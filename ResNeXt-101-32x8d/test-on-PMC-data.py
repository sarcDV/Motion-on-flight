...
for kk in range(0, len(subjects)):
    ...
    testdata =  (nib.load('./../../PMC-data-7T/PMC-ResNeXt-data/'+str(modality)+'/T1/'+str(subjects[kk])+'_T1_'+str(modality)+'_res.nii.gz')).get_fdata()
    
    ## remove upper outliers
    # testdata = remove_upper_outliers(testdata, 0.45)
    ...
    # testdata =  (nib.load('./../../PMC-data-7T/PMC-ResNeXt-data/'+str(modality)+'/T2/'+str(subjects[kk])+'_T2_'+str(modality)+'_res.nii.gz')).get_fdata()
    ## normalize 0-1:
    # testdata = testdata/testdata.max()
